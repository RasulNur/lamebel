...
export default function LangDropdown() {
    ...
    return (
        <Listbox value={selectedLang} onChange={(value) => setSelectedLang(value)} as="div" className="relative z-[91]">
            <ListboxButton
                type="button"
                ...
                {langs.map((locale) => (
                    <ListboxOption key={locale.id} value={locale} as={Fragment}>
                        {({ focus, selected }) =>
                            !selected ? (
                                <Link
                                    href={langUrl ? langUrl : pathname}
                                    locale={locale.locale}
                                    className={`flex py-2 px-4 ${focus ? "text-main" : ""} w-full`}
                                    type="button">
                                    {locale.label}
                                </Link>
                            ) : (
                                <p
                                    className={`flex py-2 px-4 ${
                                        focus ? "text-main" : ""
                                    } w-full text-white bg-main pointer-events-none`}>
                                    {locale.label}
                                </p>
                            )
                        }
